...
final class QiwoInstaller {
  enum InputMode: String, CaseIterable {
    static let primary = Self.hans
    case hans = "im.rime.inputmethod.Qiwo.Hans"
    case hant = "im.rime.inputmethod.Qiwo.Hant"
  }
  ...
  func enable(modes: [InputMode] = []) {
    ...
    if !enabledInputModes.isEmpty && modes.isEmpty {
      print("User already enabled Qiwo method(s): \(enabledInputModes.map { $0.rawValue })")
