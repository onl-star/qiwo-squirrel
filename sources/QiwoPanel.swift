...
final class QiwoPanel: NSPanel {
  private let view: QiwoView
  private let back: NSVisualEffectView
  var inputController: QiwoInputController?
  ...
  init(position: NSRect) {
    ...
    self.view = QiwoView(frame: position)
  ...
  func load(config: QiwoConfig, forDarkMode isDark: Bool) {
    if isDark {
      view.darkTheme = QiwoTheme()
      view.darkTheme.load(config: config, dark: true)
    } else {
      view.lightTheme = QiwoTheme()
      view.lightTheme.load(config: config, dark: isDark)
    }
  }
}

private extension QiwoPanel {
    ...
    if requiresFullScreen {
      ...
      if vertical {
        ...
        if position.midY / screenRect.height >= 0.5 {
          panelRect.origin.y = position.minY - QiwoTheme.offsetHeight - panelRect.height + theme.pagingOffset
        } else {
          panelRect.origin.y = position.maxY + QiwoTheme.offsetHeight
        }
        panelRect.origin.x = position.minX - panelRect.width - QiwoTheme.offsetHeight
        ...
        panelRect.origin = NSPoint(x: position.minX - theme.pagingOffset, y: position.minY - QiwoTheme.offsetHeight - panelRect.height)
      }
      ...
      if panelRect.minY < screenRect.minY {
        if vertical { panelRect.origin.y = screenRect.minY } else { panelRect.origin.y = position.maxY + QiwoTheme.offsetHeight }
      }
  ...
  func show(status message: String) {
    ...
    statusTimer?.invalidate()
    statusTimer = Timer.scheduledTimer(withTimeInterval: QiwoTheme.showStatusDuration, repeats: false) { _ in
      self.hide()
    }
  }
