...
final class QiwoInputController: IMKInputController {
  ...
  // for chord-typing
  private var chordKeyCodes: [UInt32] = .init(repeating: 0, count: QiwoInputController.keyRollOver)
  private var chordModifiers: [UInt32] = .init(repeating: 0, count: QiwoInputController.keyRollOver)
  ...
  override func handle(_ event: NSEvent!, client sender: Any!) -> Bool {
    ...
    case .flagsChanged:
      ...
      var rimeModifiers: UInt32 = QiwoKeycode.osxModifiersToRime(modifiers: modifiers)
      ...
      if !QiwoKeycode.modifierKeycodes.contains(keyCode) {
        guard let inferred = QiwoKeycode.inferModifierKeycode(from: changes) else {
      ...
      let rimeKeycode: UInt32 = QiwoKeycode.osxKeycodeToRime(keycode: keyCode, keychar: nil, shift: false, caps: false)
    ...
    case .keyDown:
      ...
      if let char = keyChars?.first {
        let rimeKeycode = QiwoKeycode.osxKeycodeToRime(keycode: keyCode, keychar: char,
  ...
  override func activateServer(_ sender: Any!) {
    ...
    var keyboardLayout = NSApp.qiwoAppDelegate.config?.getString("keyboard_layout") ?? ""
  ...
  override func hidePalettes() {
    NSApp.qiwoAppDelegate.panel?.hide()
    super.hidePalettes()
  }
  ...
  override func menu() -> NSMenu! {
    ...
    let webdavSync = NSMenuItem(title: NSLocalizedString("WebDAV Sync", comment: "Menu item"), action: #selector(webdavSync), keyEquivalent: "")
    webdavSync.target = self
    menu.addItem(webdavSync)
    let webdavSettings = NSMenuItem(title: NSLocalizedString("WebDAV Settings...", comment: "Menu item"), action: #selector(webdavSettings), keyEquivalent: "")
    webdavSettings.target = self
    menu.addItem(webdavSettings)
    menu.addItem(NSMenuItem.separator())
    menu.addItem(logDir)
  ...
  @objc func deploy() {
    NSApp.qiwoAppDelegate.deploy()
  }

  @objc func syncUserData() {
    NSApp.qiwoAppDelegate.syncUserData()
  }

  @objc func webdavSync() {
    NSApp.qiwoAppDelegate.qiwoWebDavSync()
  }

  @objc func webdavSettings() {
    NSApp.qiwoAppDelegate.openWebDavSettings()
  }

  @objc func openLogFolder() {
    NSApp.qiwoAppDelegate.openLogFolder()
  }

  @objc func openRimeFolder() {
    NSApp.qiwoAppDelegate.openRimeFolder()
  }

  @objc func checkForUpdates() {
    NSApp.qiwoAppDelegate.checkForUpdates()
  }

  @objc func openWiki() {
    NSApp.qiwoAppDelegate.openWiki()
  }
...
private extension QiwoInputController {
  ...
  func updateChord(keycode: UInt32, modifiers: UInt32) {
    ...
    chordDuration = 0.1
    if let duration = NSApp.qiwoAppDelegate.config?.getDouble("chord_duration"), duration > 0 {
  ...
  func createSession() {
    let app = client?.bundleIdentifier() ?? {
      QiwoInputController.unknownAppCnt &+= 1
      return "UnknownApp\(QiwoInputController.unknownAppCnt)"
    }()
  ...
  func updateAppOptions() {
    ...
    if let appOptions = NSApp.qiwoAppDelegate.config?.getAppOptions(currentApp) {
  ...
  func processKey(_ rimeKeycode: UInt32, modifiers rimeModifiers: UInt32) -> Bool {
    ...
    if let panel = NSApp.qiwoAppDelegate.panel {
  ...
  func rimeUpdate() {
    ...
    if rimeAPI.get_status(session, &status) {
      ...
      if let schema_id = status.schema_id, schemaId == "" || schemaId != String(cString: schema_id) {
        ...
        NSApp.qiwoAppDelegate.loadSettings(for: schemaId)
        // inline preedit
        if let panel = NSApp.qiwoAppDelegate.panel {
